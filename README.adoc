...
integers. Enter data that meets and violates the constraints. When the data is valid,
...
The `Spacecraft` bean contains 3 fields, `astronaut`, `serialNumber`, and `destination`.
...
validation logic. In this case, the serial number must start with `Liberty` followed by 4
...
violations didn't appear. Next, enter the following values, all of which break the previously defined constraints:
...
Because 3 constraint violations are defined, 3 `ConstraintViolation` objects are returned as a set
from the `validate` call. The 3 expected messages are `must be greater than 0` for the negative
