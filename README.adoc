...
one non-whitespace character. The annotation constrains the [hotspot=Name file=0]`name` field.
...
[hotspot=launch-Spacecraft file=1]`launchSpacecraft` method on the [hotspot=Spacecraft file=1]`Spacecraft` 
...
The [hotspot=launch-Spacescraft file=1]`launchSpacecraft` method uses both parameter and return value 
...
A call is then made to the [hotspot=launch-Spacecraft file=0]`launchSpacecraft` method on the 
...
that the method-level constraints that are specified on the [hotspot=launch-Spacecraft file=1]`launchSpacecraft` 
...
is made to the [hotspot=launch-Spacecraft file=1]`launchSpacecraft` method of the [hotspot=Spacecraft file=1]`Spacecraft` 
