...
== Configuring the Bean Validation Feature

Enable the bean validation feature in the application by updating the `server.xml` configuration file.
...
The [hotspot=beanValidation file=0]`beanValidation-2.0` feature definition enables the bean validation feature in the
application so that you can use it to validate that the supplied JavaBeans meet the defined constraints.
