...
non-whitespace character. The annotation is applied to the `name` field.
...
a correctly formatted email address. The annotation is applied to the `email` field.
...
constraints are applied to the `age` field to check that the astronaut is between the
ages of 18 and 100.
...
`@Positive` constraints are used to constrain the `destination` map so that the 
destination name is not blank, and the distance is positive. The `@Positive` 
constraint is used to ensure that numeric value fields are greater than 0.

* A correctly formatted serial number must be supplied. In addition to specifying the 
built-in constraints, you can create custom constraints to allow user-defined validation
rules. The `@SerialNumber` constraint that is applied to the `serialNumber` field is a 
...
not need to be respecified in the `Spacecraft` bean. Instead, because the `@Valid` 
annotation is applied to the field, all the nested constraints on the `Astronaut` bean 
are validated.
...
Additionally, the method has the `@AssertTrue` return-level constraint to 
...
The `@Constraint` annotation is used to specify the class that contains the validation 
logic for the custom constraint.

In the `SerialNumber` body, the `message()` method provides the message that is output 
when a validation constraint is violated. The `groups()` and `payload()` methods are used 
...
The `@Before` annotation is placed on a method that runs before the test cases. The
