...
Validation and the new functionality of Bean Validation 2.0. The application has a
service that uses Bean Validation to validate that the supplied JavaBeans meet the
defined constraints.
...
spacecraft bean.` section and click the `Try it out` button. Copy the following
into the text box:
...
Click `Execute` and you will receive the response `No Constraint Violations` since
the values specified pass the constraints you will create in this guide. Now try copying
...
This time you will receive `Constraint Violation Found: must be greater than or equal
to 18` as a response since the age specified was under the minimum age of 18. Try other
combinations of values to get a feel for the constraints that will be defined in this
guide.
...
section and click the `Try it out` button. Copy the following into the text box:
...
After you click `Execute`, the response will contain the following constraint
violations:
...
to note that `launched` is returned since the launch code passes the defined
...
Place the `@Before` annotation on a method that runs before the test cases. The
`setup` method retrieves the port number for the Open Liberty server and creates a
`Client`  that is used throughout the tests, which are described as follows:
