...
[hotspot=NotBlank file=0]`@NotBlank` constraint, which ensures the value is not null and contains 
...
Validation 2.0, you can specify constraints on type parameters. The [hotspot=Notblank file=1]`@NotBlank` 
...
rules. The [hotspot=SerialNum file=1]`@SerialNumber` annotation that constrains the 
...
[hotspot=launchSpacescraft file=1]`launchSpacecraft` method on the [hotspot=Spacecraft file=1]`Spacecraft` 
...
To create the custom constraint for [hotspot=SerialNum file=0]`@SerialNumber`, begin by creating 
an annotation.
...
apply the custom constraint. Because the [hotspot=serialNum file=0]`@SerialNumber` constraint is 
...
Now, create the class that provides the validation for the [hotspot=SerialNum file=0]`@SerialNumber` 
constraint.
...
The programmatic validation takes place in the [hotspot=validateSpacecraft file=2]`validateSpacecraft` 
