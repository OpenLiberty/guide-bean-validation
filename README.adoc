...
[role="code_command hotspot=11-50", subs="quotes"] 
---- 
...
The bean stores the attributes of an astronaut, [hotspot=16]`name`, [hotspot=20]`age`, 
and [hotspot=23]`emailAddress`, and provides getters and setters to access and set the 
values.

The [hotspot=11-50]`Astronaut` class has the following constraints applied:

* The astronaut needs to have a name. Bean Validation 2.0 provides a built-in 
[hotspot=15]`@NotBlank` constraint, which ensures the value is not null and contains one 
non-whitespace character. The annotation constrains the [hotspot=16]`name` field.

* The email supplied needs to be a valid email address. Another built-in constraint in 
Bean Validation 2.0 is [hotspot=22]`@Email`, which can validate that the [hotspot=11-50]`Astronaut` 
bean includes a correctly formatted email address. The annotation constrains the 
[hotspot=23]`emailAddress` field.

* The astronaut needs to be between 18 and 100 years old. Bean validation allows you to 
specify multiple constraints on a single field. The [hotspot=18]`@Min` and [hotspot=19]`@Max` 
built-in constraints applied to the [hotspot=20]`age` field check that the astronaut is 
