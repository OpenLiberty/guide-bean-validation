...
scope. To learn more about CDI see https://openliberty.io/guides/cdi-intro.html[Injecting dependencies into microservices].
...
numbers, for example `Liberty0001`. If the supplied serial number matches the constraint,
...
* `testNoFieldLevelConstraintViolations` verifies that no constraint violations occur when data is
supplied to the `Astronaut` and `Spacecraft` beans' attributes that passes the defined constraints.

* `testFieldLevelConstraintViolation` verifies that the appropriate constraint violations occur
when data supplied to the `Astronaut` and `Spacecraft` attributes violates the defined constraints.
Since there are 3 constraint violations, three `ConstraintViolation` objects are returned as a set
from the call to `validate`. The 3 expected messages are `must be greater than 0` for the negative
